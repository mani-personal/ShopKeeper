...
export function openWholesaleInvoice({
  ...
  const f = orderFinancials(order, transactions, returns, refunds),
    ...
    rows = order.items
      .map(
        (x: any, i: number) =>
          `<tr><td>${i + 1}</td><td>${escape(x.name)}</td><td>${escape(x.quantity)} ${escape(x.unit)}</td><td>${rupees(Number(x.unit_price))}</td><td>${rupees(Number(x.quantity) * Number(x.unit_price))}</td></tr>`,
      )
      .join(""),
    payments = f.transactions
      .map(
        (x: any) =>
          `<tr><td>${new Date(Number(x.created_at)).toLocaleDateString("en-IN")}</td><td>${escape(x.payment_status)}</td><td>${rupees(Number(x.amount))}</td><td>${escape(x.reference || "—")}</td></tr>`,
      )
      .join("");
