export type Product={id:string;name:string;barcode:string;category:string;subcategory?:string;weight?:string;price:number;cost:number;stock:number;min:number;unit:string;target?:number;discountMode?:"auto"|"custom"|"none"|"inherit";customDiscount?:number;specialDiscount?:boolean;discountBasis?:"cost"|"price";mrp?:number};
export type Sale={id:string;receiptNumber?:number;date:string;customer:string;payment:string;items:(Product&{qty:number})[];total:number;discount:number;productDiscount?:number;billDiscount?:number};
export function saleReceiptNumber(sales:Sale[],sale:Sale):number{
 if(sale.receiptNumber)return sale.receiptNumber;
 const used=new Set(sales.map(x=>x.receiptNumber).filter((n):n is number=>Number.isInteger(n)&&!!n&&n>0));
 let next=1;for(const old of sales.filter(x=>!x.receiptNumber).sort((a,b)=>a.date.localeCompare(b.date)||a.id.localeCompare(b.id))){while(used.has(next))next++;if(old.id===sale.id)return next;used.add(next++);}return next;
}
export const roundMoney=(n:number)=>Math.round((n+Number.EPSILON)*100)/100;
export type DiscountSettings={discountMode?:"none"|"auto"|"custom";customDiscount?:number};
export function productDiscount(p:Product,settings?:DiscountSettings):number{
...
export const netPrice=(p:Product,settings?:DiscountSettings)=>roundMoney(p.price-productDiscount(p,settings));
...
export function initial():State{return {products:[],sales:[],purchases:[],customers:[],suppliers:[],expenses:[],supplierReturns:[],supplierPayments:[],settings:{name:'My General Store',phone:'',address:'',discountMode:'none',customDiscount:0},demo:false}}
...
case 'product':{const input=a.product;const emptyPrice=input&&(input.price==null||input.price===''||Number.isNaN(input.price));const p=emptyPrice&&num(input.mrp)?{...input,price:input.mrp}:input;if(!p||!txt(p.name)||typeof p.barcode!=='string'||p.barcode.length>=200||!txt(p.category)||!txt(p.unit)||!['price','cost','stock','min'].every(k=>num(p[k]))||!Number.isInteger(p.stock)||!Number.isInteger(p.min))throw Error('Enter valid product details and whole-number stock.');if(p.barcode.trim()&&s.products.some(x=>x.barcode===p.barcode.trim()&&x.id!==p.id))throw Error('This barcode belongs to another item.');const i=s.products.findIndex(x=>x.id===p.id);if(p.target!==undefined&&(!Number.isInteger(p.target)||p.target<1||p.target>10000000))throw Error('Target stock must be a positive whole number.');const mode=p.discountMode??'none';const basis=p.discountBasis==='price'?p.price:p.cost;
if(p.discountBasis!==undefined&&!['cost','price'].includes(p.discountBasis))throw Error('Invalid discount basis.');if(p.mrp!==undefined&&(!num(p.mrp)||p.mrp<p.price))throw Error('MRP must be at least the selling price.');if(!['auto','custom','none','inherit'].includes(mode))throw Error('Choose a valid discount mode.');
...
if(p.specialDiscount!==undefined&&typeof p.specialDiscount!=='boolean')throw Error('Choose a valid special discount option.');
if(p.subcategory!==undefined&&(typeof p.subcategory!=='string'||p.subcategory.length>100))throw Error('Subcategory must be at most 100 characters.');
if(p.weight!==undefined&&(typeof p.weight!=='string'||p.weight.length>50))throw Error('Weight must be at most 50 characters.');
const clean={...(p.mrp!==undefined?{mrp:p.mrp}:{}),subcategory:(p.subcategory??'').trim(),weight:(p.weight??'').trim(),discountBasis:p.discountBasis??"cost",discountMode:mode,customDiscount:mode==='custom'?p.customDiscount:0,specialDiscount:p.specialDiscount===true,target:p.target??Math.max(p.stock,p.min*5,1),id:i>=0?p.id:id(),name:p.name.trim(),barcode:p.barcode.trim(),category:p.category,unit:p.unit,price:p.price,cost:p.cost,stock:p.stock,min:p.min};if(i>=0)s.products[i]=clean;else s.products.push(clean);break;}
case 'sale':{if(s.sales.some(x=>x.id===a.id))break;if(!txt(a.id)||!Array.isArray(a.items)||!a.items.length||!num(a.discount)||!['Cash','UPI','Card'].includes(a.payment))throw Error('Invalid sale.');if(a.items.length>200)throw Error('A bill can contain up to 200 different products.');const seen=new Set();const items=a.items.map((line:any)=>{if(seen.has(line.id)||!Number.isInteger(line.qty)||line.qty<1||line.qty>1000000)throw Error('Enter valid, unique bill items and quantities.');seen.add(line.id);if(line.manual===true){if(typeof line.id!=='string'||!/^manual:[a-f0-9-]{36}$/.test(line.id)||!txt(line.name)||!num(line.unitPrice)||line.unitPrice<=0||!num(line.cost)||roundMoney(line.unitPrice)!==line.unitPrice||roundMoney(line.cost)!==line.cost)throw Error('Enter a manual item name, selling price and purchase cost.');return {id:line.id,name:line.name.trim(),barcode:'',category:'Manual sale',unit:'piece',stock:0,min:0,price:line.unitPrice,cost:line.cost,discountMode:'none' as const,customDiscount:0,qty:line.qty};}const p=s.products.find(x=>x.id===line.id);if(!p||line.qty>p.stock)throw Error('Stock changed. Refresh and check bill quantities.');if(line.unitPrice!==undefined&&(!num(line.unitPrice)||roundMoney(line.unitPrice)!==line.unitPrice||(p.mrp!==undefined&&line.unitPrice>p.mrp)))throw Error('Enter a valid selling price no higher than MRP.');return line.unitPrice!==undefined?{...p,price:line.unitPrice,discountMode:"none" as const,customDiscount:0,qty:line.qty}:{...p,discountMode:"custom" as const,customDiscount:productDiscount(p,s.settings),qty:line.qty}});const subtotal=roundMoney(items.reduce((t:number,p:any)=>t+p.price*p.qty,0));const savings=roundMoney(items.reduce((t:number,p:any)=>t+productDiscount(p)*p.qty,0));if(a.discount>roundMoney(subtotal-savings))throw Error('Bill discount cannot exceed the total after product discounts.');items.filter((p:any)=>!p.id.startsWith('manual:')).forEach((p:any)=>{s.products.find(x=>x.id===p.id)!.stock-=p.qty});const usedNumbers=new Set(s.sales.map(x=>x.receiptNumber).filter((n):n is number=>Number.isInteger(n)&&!!n&&n>0));let legacyNumber=1;for(const old of s.sales.filter(x=>!x.receiptNumber).sort((a,b)=>a.date.localeCompare(b.date)||a.id.localeCompare(b.id))){while(usedNumbers.has(legacyNumber))legacyNumber++;old.receiptNumber=legacyNumber;usedNumbers.add(legacyNumber++);}const receiptNumber=s.sales.reduce((max,x)=>Math.max(max,x.receiptNumber??0),0)+1;s.sales.unshift({id:a.id,receiptNumber,date:new Date().toISOString(),customer:typeof a.customer==='string'?a.customer.slice(0,200):'Walk-in customer',payment:a.payment,items,total:roundMoney(subtotal-savings-a.discount),discount:roundMoney(savings+a.discount),productDiscount:savings,billDiscount:a.discount});break;}
...
case 'settings':if(!['none','auto','custom'].includes(a.discountMode??s.settings.discountMode??'none')||!num(a.customDiscount??s.settings.customDiscount??0)||roundMoney(a.customDiscount??s.settings.customDiscount??0)!==(a.customDiscount??s.settings.customDiscount??0))throw Error('Choose a valid store discount and custom amount.');if(!txt(a.name)||typeof a.phone!=='string'||typeof a.address!=='string'||a.address.length>500)throw Error('Enter valid store details.');if(!num(a.lowPercent)||a.lowPercent>100)throw Error('Low-stock percentage must be between 0 and 100.');s.settings={name:a.name.trim(),phone:a.phone.slice(0,30),address:a.address,lowPercent:a.lowPercent,discountMode:a.discountMode??s.settings.discountMode??'none',customDiscount:a.customDiscount??s.settings.customDiscount??0};break;
