...
export function AccountPassword({title="Change password",email}:{title?:string;email?:string}={}) {
  const [message, setMessage] = useState("");
  const [busy, setBusy] = useState(false);
  return <section className="panel padded"><h2>{title}</h2>{email&&<p>{email}</p>}<p>Enter your current password to update this account. Other sessions will be signed out.</p>
