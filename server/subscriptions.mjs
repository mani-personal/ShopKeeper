...
export async function subscriptionInfo(db,v,config){return {...validity(v,config.trialDays),history:await db.prepare('SELECT * FROM subscription_history WHERE vendor_id=? ORDER BY created_at DESC,id DESC LIMIT 200').all(v.id)};}
