...
export async function allowedVendors(db, user) { return user.role === 'owner' ? await db.prepare('SELECT * FROM vendors ORDER BY rowid').all() : await db.prepare('SELECT v.* FROM vendors v JOIN memberships m ON m.vendor_id=v.id WHERE m.user_id=? AND v.suspended=FALSE ORDER BY v.rowid').all(user.id); }
export async function requireVendor(db, user, id) { if (typeof id !== 'string' || !id)
    ...
    throw Object.assign(Error('Store not found or access denied.'), { status: 403 }); if(user.role !== 'owner' && row.suspended)throw suspensionError(); return row; }
