...
export async function readSession(db, req) { const raw = req.headers.cookie?.split(';').map(x => x.trim()).find(x => x.startsWith('shop_session='))?.slice(13); if (!raw)
    return null; return await db.prepare('SELECT users.id,users.email,users.role,users.name,sessions.csrf,sessions.hash FROM sessions JOIN users ON users.id=sessions.user_id WHERE sessions.hash=? AND sessions.expires>? AND users.disabled=FALSE').get(digest(raw), Date.now()) ?? null; }
...
export function requireOwner(user) { if (!isAdmin(user))
