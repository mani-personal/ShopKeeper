...
export async function bootstrap(db, config) { if (await db.prepare("SELECT id FROM users WHERE role='owner'").get())
    ...
    return false; await db.prepare('INSERT INTO users(id,email,password_hash,role,name,created_at) VALUES(?,?,?,?,?,?)').run(randomUUID(), mail, hash, 'owner', 'Administrator', Date.now()); const book = upgradeBook(initial()); for (const v of config.seedDemo ? book.vendors : book.vendors.slice(0, 1)) {
